...
if ( ! class_exists( 'Example_Widget' ) ) {
    ...
    class Example_Widget extends WP_Widget {
        ...
        public function __construct() {
            parent::__construct(
                'Example_Widget',
                __('My Youtube Recommendation' , 'example'),
                array(
        ...
        public function form( $instance ) {	
            ...
            <?php // Title ?>
            <p>
                <label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php echo __('Title' , 'example'); ?>:</label>
            ...
            <?php // Layout ?>
            <p>
                <label for="<?php echo esc_attr( $this->get_field_id( 'layout' ) ); ?>"><?php echo __('Layout' , 'example'); ?>:</label>
                <select class="postform" id="<?php echo esc_attr( $this->get_field_id( 'layout' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'layout' ) ); ?>">
                <option class="level-0" value="grid" <?php echo (esc_attr( $layout ) == 'grid') ? 'selected="selected"': '' ?>><?php echo __('Grid' , 'example') ?></option>
                <option class="level-0" value="list" <?php echo (esc_attr( $layout ) == 'list') ? 'selected="selected"': '' ?>><?php echo __('List' , 'example') ?></option>
                </select>
            </p>

            <?php // Limit ?>
            <p>
                <label for="<?php echo esc_attr( $this->get_field_id( 'limit' ) ); ?>"><?php echo __('Videos to show' , 'example'); ?>:</label>
        ...
        public function widget( $args, $instance ) {
            ...
            <div class="widget-text wp_widget_plugin_box">
                ...
                <div id='<?php echo $widget_unique_id ?>'><?php echo __('Loading...' , 'example') ?></div>
            </div>
            <script>
