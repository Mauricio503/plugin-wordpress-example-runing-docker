...
class Example {
		...
		private function build_html_videos_list() {
			...
			$content        .= "<div id='$container_id'>".__('Loading...' , 'example')."</div>";
			$script         = "<script>
		...
		public function enqueue_assets() {
	        ...
	        wp_enqueue_style( 'example-style', plugin_dir_url( __DIR__ ) . 'public/css/style.css' );
        	wp_enqueue_script( 'example-scripts', plugin_dir_url( __DIR__ ) . 'public/js/scripts.js', array( 'jquery' ), '', false );
        	wp_enqueue_script( 'example-loader', plugin_dir_url( __DIR__ ) . 'public/js/loader.js', array( 'jquery' ), '', true );
			wp_localize_script( 'example-scripts', 'my_yt_rec_ajax', array( 'url' => network_admin_url( 'admin-ajax.php' ) ) );
